...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://alaskapi.onrender.com/login", {
        method: "POST",
        headers: {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-sky-100 to-white px-4">
      <div className="w-full max-w-md space-y-8 bg-white p-8 rounded-2xl shadow-lg">
        <div className="flex flex-col items-center">
          ...
          />
          <h2 className="text-center text-3xl font-bold tracking-tight text-gray-900">Bem-vindo de volta</h2>
          <p className="mt-2 text-center text-sm text-gray-600">Por favor, faça login na sua conta</p>
        </div>
        ...
        <div className="mt-6">
          <p className="text-center text-sm text-gray-600">
            Não tem uma conta?{" "}
            <a href="/cadastro" className="font-medium text-sky-600 hover:text-sky-500">
              Cadastre-se
            </a>
          </p>
        </div>
